...
function Peer(options) {
  ...
  this.filterloadSent = false;
  // set message handlers
  var self = this;
  this.on('verack', function() {
    self.status = Peer.STATUS.READY;
    self.emit('ready');
    if(!self.filterloadSent){
      self._sendFilterload();
    }
  });
...
Peer.prototype._sendFilterload = function() {
  var args={};
  args.vData=new Array();
  args.nHashFuncs=0;
  //console.log(args);
  var message = this.messages.FilterLoad(BloomFilter.create(3, 0.01, 0, BloomFilter.BLOOM_UPDATE_ALL));
  this.filterloadSent = true;
  this.sendMessage(message);
};
...
Peer.prototype._readMessage = function() {
  ...
  if (message) {
    //console.log("read message: "+message.command);
