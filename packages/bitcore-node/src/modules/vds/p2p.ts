...
import { VdsBlock, VdsBlockStorage, IVdsBlock } from '../../models/vdsBlock';
...
import { VdsBlockType, BitcoinHeaderObj, VdsTransaction } from '../../types/namespaces/Vds';
import { wait } from '../../utils/wait';

export class VdsP2PWorker extends BaseP2PWorker<IVdsBlock> {
  ...
  protected blockModel: VdsBlock;
  protected pool: any;
  protected syncInterval?: NodeJS.Timer;
  public events: EventEmitter;
  public isSyncing: boolean;
  constructor({ chain, network, chainConfig, blockModel = VdsBlockStorage }) {
  ...
  public async getBlock(hash: string) {
    ...
    let received = false;
    return new Promise<VdsBlockType>(async resolve => {
      this.events.once(hash, (block: VdsBlockType) => {
  ...
  async processBlock(block: VdsBlockType): Promise<any> {
  ...
  async processTransaction(tx: VdsTransaction): Promise<any> {
    const now = new Date();
  ...
  async sync() {
    ...
    //console.log("sync getHeaders==================\n\n\n\n\n\n\n\n")
    //console.log(headers)

    while (headers.length > 0) {
      tip = await ChainStateProvider.getLocalTip({ chain, network });
      let currentHeight = tip ? tip.height : 0;
      //console.log("loop get Headers"+currentHeight);
      ...
      for (const header of headers) {
        try {
          //console.log("now get Header hash:"+header.hash);
          const block = await this.getBlock(header.hash);
          //console.log(block);
          await this.processBlock(block);
          currentHeight++;
          //console.log("curHeight add:"+currentHeight);
          const now = Date.now();
