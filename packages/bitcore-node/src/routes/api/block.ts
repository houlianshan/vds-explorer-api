...
import { TransactionStorage } from '../../models/vdsTransaction';
...
router.get('/:blockHash/coins/:limit/:pgnum', async function(req: Request, res: Response) {
  ...
  try {
    ...
    let values = txs.map((tx) => {
      return {value:tx.value,fee:tx.fee};
    });
    ...
    return res.json({ txids,values, inputs: sanitize(inputs), outputs: sanitize(outputs), previous, next });
  } catch (err) {
    return res.status(500).send(err);
  }
});
