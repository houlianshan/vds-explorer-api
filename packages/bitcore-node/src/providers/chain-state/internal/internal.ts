...
export class InternalStateProvider implements IChainStateService {
  ...
  async streamAddressTransactions(params: StreamAddressUtxosParams) {
    const { req, res, args } = params;
    const { limit, since,skip } = args;
    const query = this.getAddressQuery(params);
    let sort={ mintBlockTime:-1,spentBlockTime:-1 }
    Storage.apiStreamingFind(CoinStorage, query, { limit, since,skip,sort}, req, res);
  }
  ...
  async getLocatorHashes(params) {
    ...
    if (locatorBlocks.length < 2) {
      // if(network=="testnet"){
      //   return ["bd94031d0ba5bbb72b50eecd5f5444056e5f0f788538e24261878178cdab6a62"]
      // }else{
