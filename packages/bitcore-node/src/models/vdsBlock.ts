...
import { VdsBlockType, BitcoinHeaderObj } from '../types/namespaces/Vds';
import { TransformOptions } from '../types/TransformOptions';
import { MongoBound } from './base';
import { BaseBlock, IBlock } from './baseBlock';
import { CoinStorage, ICoin } from './coin';
import { EventStorage } from './events';
import { TransactionStorage } from './vdsTransaction';

export type IVdsBlock = IBlock & {
  version: number;
  merkleRoot: string;
  bits: number;
  nonce: number;
  coinbaseCoins ?: Array<ICoin>
};

@LoggifyClass
export class VdsBlock extends BaseBlock<IVdsBlock> {
  ...
  async getBlockOp(params: { block: VdsBlockType; chain: string; network: string }) {
    const { block, chain, network } = params;
    const header = block.header.toObject();
    ...
    const convertedBlock: IVdsBlock = {
      chain,
      network,
      hash: block.hash,
      height,
  ...
  _apiTransform(block: Partial<MongoBound<IVdsBlock>>, options?: TransformOptions): any {
    const transform = {
      _id: block._id,
      chain: block.chain,
...
export let VdsBlockStorage = new VdsBlock();
