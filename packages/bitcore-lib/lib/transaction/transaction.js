...
Transaction.prototype.toBufferWriter = function(writer, noWitness) {
  ...
  writer.writeUInt32LE(this.exp);
  writer.writeUInt64LEBN(this.vBalance);

  writer.writeVarintNum(this.spendDescriptions.length);
  _.each(this.spendDescriptions, function(spendDescription) {
    spendDescription.toBufferWriter(writer);
  });

  writer.writeVarintNum(this.outputDescriptions.length);

  _.each(this.outputDescriptions, function(outputDescription) {
    outputDescription.toBufferWriter(writer);
  });

  writer.write(this.sigt);
  return writer;
};
...
Transaction.prototype.fromBufferReader = function(reader) {
  ...
  this.flag=reader.read(1).readUInt8();
  ...
  this.exp = reader.readUInt32LE();
  this.vBalance=reader.readUInt64LEBN();


  var spendDescriptionSize = reader.readVarintNum();

  for (var i = 0; i < spendDescriptionSize; i++) {
    var spenddescription = SpendDescription.fromBufferReader(reader);
    this.spendDescriptions.push(spenddescription);
  }


  var outputDescriptionSize = reader.readVarintNum();

  for (var i = 0; i < outputDescriptionSize; i++) {
    var outputDescription = OutputDescription.fromBufferReader(reader);
    this.outputDescriptions.push(outputDescription);
  }

  this.sigt=reader.read(64);

  //throw new Error("pause transaction write");
  return this;
};
