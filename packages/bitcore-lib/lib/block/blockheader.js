...
var BlockHeader = function BlockHeader(arg) {
  ...
  this.hashFinalSaplingRoot=info.hashFinalSaplingRoot;
  this.mVibPool=info.mVibPool;
  this.time = info.time;
  ...
  this.solutionBytes=info.solutionBytes;
...
BlockHeader._fromObject = function _fromObject(data) {
  ...
  var hashFinalSaplingRoot=data.hashFinalSaplingRoot;
  ...
  if (_.isString(data.hashFinalSaplingRoot)) {
    hashFinalSaplingRoot = BufferUtil.reverse(Buffer.from(data.hashFinalSaplingRoot, 'hex'));
  }
...
BlockHeader._fromBufferReader = function _fromBufferReader(br) {
  ...
  info.hashFinalSaplingRoot=br.read(32);
  info.mVibPool=br.readUInt64LEBN();
  info.time = br.readUInt32LE();
  info.bits = br.readUInt32LE();
  info.hashStateRoot=br.read(32);
  info.hashUTXORoot=br.read(32);
  info.nonce = br.read(32);
  info.solutionBytes=br.readVarLengthBuffer();
  return info;
};
...
BlockHeader.prototype.toBufferWriter = function toBufferWriter(bw) {
  ...
  bw.write(this.hashFinalSaplingRoot);
  bw.writeUInt64LEBN(this.mVibPool);
  bw.writeUInt32LE(this.time);
  bw.writeUInt32LE(this.bits);
  bw.write(this.hashStateRoot);
  bw.write(this.hashUTXORoot);
  bw.write(this.nonce);
  bw.writeVarintNum(this.solutionBytes.length);
  bw.write(this.solutionBytes);
  return bw;
};
